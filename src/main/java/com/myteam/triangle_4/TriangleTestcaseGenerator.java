...
//    }
    ...
    public static List<TriangleTestcase> generateTestCase(int input, int max, int min) {
        ...
        for (int i = 0; i < side.get(2).length; i++) {
            ...
            expected = classifyTriangle(side1, side2, side3, max, min);
    ...
    public static String classifyTriangle(int side1, int side2, int side3, int max, int min) {
        if (side1 <= 0 || side2 <= 0 || side3 <= 0 || side1 > max || side2 > max || side3 > max) {
            return "Out of range";
